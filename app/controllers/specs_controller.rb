class SpecsController < ApplicationController
  ...
  def index
    param_set
    @count	= Spec.notnil().includes(:p_name, :condition_all_text).search(params[:q]).result.count()
    @search	= Spec.notnil().includes(:p_name, :condition_all_text).page(params[:page]).search(params[:q])
