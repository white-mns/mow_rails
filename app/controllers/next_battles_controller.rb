class NextBattlesController < ApplicationController
  ...
  def index
    param_set
    @count	= NextBattle.includes(:p_name, :status, :spec, :reward, :condition_all_text).search(params[:q]).result.count()
    @search	= NextBattle.includes(:p_name, :status, :spec, :reward, :condition_all_text).page(params[:page]).search(params[:q])
  ...
  def param_set
    ...
    @show_detail_2_1 = params["show_detail_2_1"]
